...
Public marketing site for APEX Gainesville. **This site is built.** What comes next is almost always a small change from someone who is **not an engineer**, will not use engineering words, and is talking to **Claude** (not Cursor).

They will not say “edit `src/content/pricing.ts`”, “add a token”, or “compose a pattern”. They will say “the $60 is now $70”, “Tier copy is wrong”, or “add this to the FAQ”.
...
Every class, schedule, and payment lives in **Arketa**. This site never books, charges, or holds a membership — it links and it embeds. If someone reports “booking is broken” or “the schedule is empty”, that is almost always Arketa, not this repo. Check `src/content/checkout.ts` for a wrong id before you change anything else.
...
| “the $60 is now $70” / “new checkout link” | `src/content/checkout.ts` | Paste an Arketa URL in a button |
...
They are in **Claude**. Skills live in `.claude/skills/` — pick the one whose **description** sounds like what they said. Always-on and path rules live alongside the code as `AGENTS.md` files in each folder.

## They said → you do

| They said something like | What it is | Where |
| --- | --- | --- |
| “the $60 is now $70” / “new checkout link” | price or Arketa id | `src/content/checkout.ts` |
| “add a class” / “FLEX is 25 now” | class styles | `src/content/classes.ts` |
...
| “make it warmer / less black / bigger type” | theme | `src/styles/palette.css` + `theme.css` |
...
| “deploy” / “go live” | ship | `.github/DEPLOY.md` |
...
4. Big titles use `font-display` (Oswald). Body is `font-body` (Nunito Sans). Both are self-hosted in `public/fonts/`.
...
7. Public URLs keep their Squarespace paths (`/the-workout/`, `/pricing/`, `/schedule/`). Renaming one breaks search rankings and every link in past emails. In-page jumps like `#intro` are fine.
...
10. Hex lives only in `palette.css`. This palette is APEX's own. Do not copy it to the RAD or yoga pod repos, and do not copy theirs here.
...
`/cancel/`, `/freeze/`, `/upgrade/` and `/downgrade/` are the only membership forms on the
site (plus `/help/` and the footer mailing list). They post to Netlify, not to Arketa — a person reads them and makes the change by hand.
...
`forms-check: 6 forms declared and rendered consistently`.
...
- Arketa ids outside `src/content/checkout.ts`
...
git switch -c drop-in-70        # short name for the change
```
...
4. `git commit -m 'Raise the drop-in to $70.'` — one plain sentence, like the history. Use single quotes here and in the pull request: inside double quotes, the shell mangles or drops `$70`.
5. `git push -u origin HEAD`
6. No pull request for this branch yet? Open one: `gh pr create --base main --title '…' --body '…'`. In the body, say what changed in plain words and flag anything a human should double-check (a price, a checkout id, a form field, a public URL). If `gh` is missing or not signed in, give them `https://github.com/ajjarboe/APEX-gainesville/pull/new/<branch>` instead.
